...
import rainConfig from "../configs/rain-data.json";

export default class Raindrops {
	constructor(canvasUtils, dropAlpha, dropColor) {
		this.ctx = null;
		...
		this.width = this.canvasUtils.canvas.width;
		this.height = this.canvasUtils.canvas.height;
		this.scale = this.canvasUtils.dpi;
		this.dropAlpha = dropAlpha;
		this.dropColor = dropColor;
		this.options = Object.assign({}, rainConfig.raindrops.defaultOptions, rainConfig.raindrops.options);
		this.dropSize = rainConfig.raindrops.dropSize;
		this.init();
	}

	init() {
		this.canvas = this.canvasUtils.createCanvas(this.width, this.height);
		this.ctx = this.canvas.getContext('2d');

		this.droplets = this.canvasUtils.createCanvas(this.width * this.dropletsPixelDensity, this.height * this.dropletsPixelDensity);
	...
	drawDroplet(x, y, r) {
		this.drawDrop(this.dropletsCtx, Object.assign(Object.create(rainConfig.raindrops.drop), {
	...
	renderDropsGfx() {
		let dropBuffer = this.canvasUtils.createCanvas(this.dropSize, this.dropSize);
		let dropBufferCtx = dropBuffer.getContext('2d');
		this.dropsGfx = Array.apply(null, Array(255))
			.map((cur, i) => {
				let drop = this.canvasUtils.createCanvas(this.dropSize, this.dropSize);
		...
		this.clearDropletsGfx = this.canvasUtils.createCanvas(128, 128);
	...
	createDrop(options) {
		...
		return Object.assign(Object.create(rainConfig.raindrops.drop), options);
	}
